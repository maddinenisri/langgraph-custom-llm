...
if (!WORKSPACE_DIR) {
  console.warn(
    "WORKSPACE_DIR environment variable not set. File system tools will be disabled."
  );
} else {
  // Ensure workspace directory exists on startup (optional, but good practice)
  fs.mkdir(WORKSPACE_DIR, { recursive: true }).catch((err) => {
    console.error(
      `Error creating workspace directory '${WORKSPACE_DIR}':`,
      err
    );
...
function resolveWorkspacePath(relativePath: string): string | null {
  ...
  if (path.isAbsolute(relativePath) || relativePath.includes("..")) {
    console.warn(
      `Security Alert: Attempted file access outside workspace: ${relativePath}`
    );
  ...
  if (!fullPath.startsWith(path.resolve(WORKSPACE_DIR))) {
    console.warn(
      `Security Alert: Resolved path escaped workspace: ${fullPath}`
    );
    return null; // Indicate invalid path
  }
  ...
  },
  {
    ...
    schema: z.object({
      filePath: z
        .string()
        .describe(
          `Relative path to the file within the workspace (e.g., "notes.txt", "code/main.py").`
        ),
    }),
  }
);
  ...
  },
  {
    ...
    schema: z.object({
      filePath: z
        .string()
        .describe(
          `Relative path to the file within the workspace (e.g., "new_notes.md", "data/output.csv").`
        ),
...
const calculatorTool = tool(
  ...
  },
  {
    name: "calculator",
    description:
      "Useful for performing simple arithmetic calculations (add, subtract, multiply, divide).",
    schema: z.object({
      operation: z
